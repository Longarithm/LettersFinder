...
map<long long, int> CountOccurences(int M, int N, long long hash_MxN[HEIGHT][WIDTH]) { 
	...
	for (int i = 0; i < height ; i++) {
		for (int j = 0; j < width ; j++) {
			number_of_occur[hash_MxN[i][j]]=0;
		}
	}
...
void FindFixedRectangles(int M, int N) { 
	...
	for (auto it = number_of_occur_MxN.begin(); it != number_of_occur_MxN.end(); it++) {
		pair<long long, int> t = *it;
		if (t.second >= 5)
			printf("%d %d\n", t.first, t.second);
	}
	...
	for (int i = 0; i < height - M + 1; i++) {
		for (int j = 0; j < width - N + 1; j++) {
			...
			if ((5 < number_of_occur_MxN[hash_MxN1[i][j]]) && (number_of_occur_MxN1[hash_MxN[i][j]] < 5000)) {

            //      (number_of_occur_MxN1[hash_MxN1[i][j]] <  number_of_occur_MxN[hash_MxN[i][j]])
			...
			//      && 
				...
				//or just print it
				printf("i=%d j=%d %d %d is a distingished rectangle occ %d<%d\n", i + 1, j + 1, i + M, j + N, number_of_occur_MxN1[hash_MxN1[i][j]],number_of_occur_MxN[hash_MxN[i][j]]); 
